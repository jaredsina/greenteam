...
class Conflict extends StatefulWidget {
  final String title = 'Conflict';

  const Conflict({super.key});

  @override
  _ConflictState createState() => _ConflictState();
}

class _ConflictState extends State<Conflict> {
  ...
  Future<void> _fetchLatestNote() async {
    try {
      final url = Uri.parse('http://127.0.0.1:4000/notes/get/conflict');
      final response = await http.get(url);
              ...
              children: [
                FilledButton.tonal(
                  ...
                  onPressed: () {
                    ...
                    createPost(context, 'conflict', jsoncontent);
                  },
                  child: const Center(
                    child: Text(
                      'Save',
