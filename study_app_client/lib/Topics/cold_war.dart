...
class ColdWar extends StatefulWidget {
  final String title = 'Cold War';

  const ColdWar({super.key});

  @override
  _ColdWarState createState() => _ColdWarState();
}

class _ColdWarState extends State<ColdWar> {
  ...
  Future<void> _fetchLatestNote() async {
    try {
      final url = Uri.parse('http://127.0.0.1:4000/notes/get/cold_war');
      final response = await http.get(url);
              ...
              children: [
                FilledButton.tonal(
                  ...
                  onPressed: () {
                    ...
                    createPost(context, 'cold_war', jsoncontent);
                  },
                  child: const Center(
                    child: Text(
                      'Save',
