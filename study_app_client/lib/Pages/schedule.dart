...
Future<void> createPost(
  ...
  if (response.statusCode == 201) {
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('Schedule created and saved.')));
  } else {
    ScaffoldMessenger.of(
      context,
        ...
        .then((data) {
          setState(() {
            ...
            for (var sched in data) {
              ...
              final int startHour = (sched['start_time'] is int)
                  ? sched['start_time']
                  : (int.tryParse('${sched['start_time']}') ?? 0);
              final int endHour = (sched['end_time'] is int)
                  ? sched['end_time']
                  : (int.tryParse('${sched['end_time']}') ?? (startHour + 1));

              final DateTime startTime =
                  date?.add(Duration(hours: startHour)) ?? DateTime.now();
              final DateTime endTime =
                  date?.add(Duration(hours: endHour)) ??
